 ...
 *    backdrop. Every semantic surface on this page is one. `--c-safe-bg`,
 *    `--c-warn-bg` and `--c-danger-bg` are `rgba(...)` tints at 7–10% alpha
 *    over whatever card is behind them, and they carry the states this lab
 *    exists to show: every `.result-box-success` / `.result-box-error` verdict,
 *    every `.status-chip`, every `.alert`, the `.byte-cell.known` cells of the
 *    Bleichenbacher recovery grid, and the `.factor-side-tag` pair on the
 *    trapdoor. The shared top bar adds a `color-mix(in srgb, ...)` for its ink.
 *    A violations-only gate therefore measured the contrast of almost none of
 *    this lab's generated output.
 *
 ...
 *    at `.5` (exempt as an inactive component, and skipped below for that
 *    reason) and the hero subtitle.
 *
 ...
 *    eight `overflow-x: auto` containers — the six-tab `.tab-list`, the
 *    comparison table, the byte grids, the padding diagrams, the oracle log —
 *    and at 380px most of them really do clip. Content scrolled past one is not
 *    dimmed or partly drawn; it is absent from the frame, and asking what colour
 *    it sits on has no answer. Its rect is outside every ancestor's box, so the
 ...
 *    draws its diagrams in HTML and CSS grid; its only SVG is the shared
 *    header's three glyph-free `aria-hidden` marks, two of which are built from
 *    stroke-only `<line>` elements. That is exactly the shape the `FILLED` guard
 *    below exists for: SVG's initial `fill` is black and `getComputedStyle`
 *    reports it for stroke-only geometry too, so an unguarded underlay walk
 *    would treat each hamburger rule as an opaque black rectangle.
 *
 ...
 * `content` exactly once, `.status-chip::before { content: '' }`, an empty
 * 7px dot filled with `currentColor` — the chip's own ink, on the chip's own
 * background, so it is covered by whatever ratio the chip's TEXT already has to
 * clear, which is the stricter 4.5:1 rather than 1.4.11's 3:1.
 *
 * SECOND, `aria-hidden` text that is still painted — see the `ariaHidden` note
 * below. This page hides four elements that carry WORDS, so `gate.ts` measures
 * every `aria-hidden` subtree explicitly with that exemption lifted.
 */
 ...
 * inside an `aria-hidden` subtree still has to clear 4.5:1 — yet axe skips it
 * and, by default, so does this walk. This page hides four elements that carry
 * words (`EASY`, `IMPOSSIBLE`, `THE LEVER`, `→ scramble →`), each painted in a
 * semantic ink on its own low-alpha tint, so `scan()` calls this a second time
 * as `auditContrast(page, '[aria-hidden="true"], [aria-hidden="true"] *', true)`.
 */
export async function auditContrast(
  ...
  return page.evaluate(([rootSelector, allowAriaHidden]: [string, boolean]) => {
     ...
     * the byte grids: 128 `span.det-byte` cells in the determinism contrast,
     * 24 `span.oaep-hl-byte` in the avalanche block, and 16 `div.byte-cell` each
     * holding two spans in the Bleichenbacher recovery row — all of them
     * siblings re-walking the same ancestors up to `<body>`, alongside a growing
     * oracle log. Without the caches the pass re-reads the same computed styles
     ...
     * those are the eight `overflow-x: auto` containers: `.tab-list`,
     * `.table-wrap`, the byte-grid rows, the OAEP and PSS padding diagrams and
     * the oracle log — several of which genuinely clip at 380px, and one of
     * which (`.sr-only`) is handled separately below.
     */
    ...
    const svgUnderlay = (el: Element, box: DOMRect): RGBA => {
      ...
      // ratio was 6.15:1. This page's only SVG is the shared header's three
      ...
      // draws a labelled figure of its own.
    ...
    const isVisible = (el: Element): boolean => {
      ...
      // `<details class="oaep-details">` wrapped around the OAEP padding
      // diagram — the only disclosure on the page, and it ships shut. The gate
      // opens it by clicking its <summary>, which is the route a reader has,
      // rather than setting `.open` from script — which is what the gate this
      // replaces did, to every <details> on the page, before its only scan.
      ...
      //
      // This lab uses that idiom heavily, which is why the guard is load-bearing
      // here rather than merely inherited. `styles/main.css` defines `.sr-only`
      // as `position:absolute; width:1px; height:1px; clip:rect(0,0,0,0)`, and
      // FIVE elements carry it: `#aria-live` and `#aria-live-assertive`, plus
      // the `#tb-keygen-status`, `#oaep-keygen-status` and `#pss-keygen-status`
      // `role="status"` regions. Every announcement this lab makes lands in one
      // of them — and this lab reports ALL of its soft errors that way, so their
      // text changes on almost every step of the drive. Each has a rect and
      // passes `checkVisibility()` while painting exactly zero pixels; measuring
      // them would report a fresh ratio for ink that was never laid down, on
     ...
     *
     * There are 29 `aria-hidden` elements on this page. Most are decorative
     * glyphs sitting immediately beside the words that carry their meaning, in
     * the same ink on the same surface (🔬 🔭 💡 ⚠️ ℹ️ ✓ 🔓 🔒 🌙 and the `—`
     * placeholders), plus two empty ones — `.tab-status-dot`, a 7px colour dot
     * that duplicates the `.status-chip` text inside each panel, and the
     * `.loading-spinner` that `setLoading` prepends into a busy button.
     *
     * FOUR of them carry real words and nothing else does: `EASY` and
     * `IMPOSSIBLE` (`.factor-side-tag`, painted `--c-safe` / `--c-danger` on
     * their own tint), `THE LEVER` (`.bb-homomorphic-tag`), and `→ scramble →`
     * (`.oaep-hl-arrow`). Those are the reason `scan()` runs the whole
     * `aria-hidden` set through this walk with the exemption lifted.
     *
     * Nothing on this page hides a VALUE. No ciphertext, signature, recovered
     * byte or verdict is inside an `aria-hidden` subtree — which was checked
     ...
     *
     * This lab's only SVG is the shared header's three `aria-hidden` marks, none
