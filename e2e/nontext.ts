 ...
 *
 * IN THIS LAB the control-boundary half is the live one, and the shape is the
 * fleet's: `styles/main.css` defines `--c-border-strong` under a comment that
 * says "Control boundaries only (WCAG 2.1 SC 1.4.11)" and then uses it in
 * exactly ONE rule — `.form-input, .form-textarea` — while `--c-border`, the
 * decorative surface divider, appears 44 times, including on `.btn-ghost`.
 * A token defined for control boundaries, applied to one control.
 *
 * The generated-content half has one candidate here, `.status-chip::before`,
 * and it is not a defect: `content: ''` with `background: currentColor`, so the
 * dot is the chip's own ink on the chip's own surface and inherits whatever
 * ratio the chip's TEXT already has to clear — 4.5:1, stricter than the 3:1
 * this check would ask of it. The check stays because the next author-declared
 * `content` on this page will not be empty.
 *
...
export async function auditNonText(page: Page, within = 'body *'): Promise<NonTextFailure[]> {
  return page.evaluate((rootSelector) => {
    ...
    for (const el of Array.from(document.querySelectorAll(CONTROL))) {
      ...
      // PER SIDE, not `border-top` for all four. An earlier form of this read
      // `border-*-width` on every side but then measured `borderTopColor` alone,
      // which silently measures the WRONG EDGE whenever a control is delineated
      // by one side only. This page has exactly that: `.tab-btn` is
      // `border: none; border-bottom: 3px solid transparent`, and the selected
      // tab paints `border-bottom-color: var(--c-primary)` — a 3px sky-blue
      // underline against the nav bar, which is the entire visual boundary of
      // the control and is the ARIA tab pattern's normal one. Reading
      // `borderTopColor` there returns `transparent`, the border branch
      // collapses to the fill, and the oracle reported 1.12:1 for a control
      // whose real delineator is far above 3:1.
      //
      // A side also has to be OPAQUE ENOUGH TO PAINT. `border: 2px solid
      // transparent` is a layout spacer, not a delineator — it reserves the
      // 2px a coloured state will later occupy so nothing shifts. This page
      // uses it on every `.btn` and on every `.tab-btn`, and counting it as a
      // border made the five UNSELECTED tabs — which have no fill and no
      // painted edge, and identify themselves by their text alone, exactly the
      // case the "is it trying to draw itself as a control?" test below exists
      // to exclude — report 1.00:1 apiece.
